...
export default function LecturerViolationsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const exams = await getExams(lecturer.id);
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div>
        ...
        <p className="text-sm text-[var(--text-secondary)] mt-1">Examination violations for your courses</p>
      </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-sm text-[var(--text-secondary)]">
                    No violations recorded for your examinations
                  </td>
                </tr>
              ) : (
                filtered.map((v) => (
