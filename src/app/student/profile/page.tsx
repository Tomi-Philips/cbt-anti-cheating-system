...
import { getCurrentStudent, getCurrentUser } from "@/lib/actions";
...
export default function StudentProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [s, p] = await Promise.all([getCurrentStudent(), getCurrentUser()]);
        setStudent(s);
        setProfile(p);
      } catch {
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <Card className="max-w-2xl">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium text-[var(--text-secondary)] uppercase">Student ID</label>
              <p className="text-sm font-medium text-[var(--text)] mt-1">{student?.student_id || "—"}</p>
            </div>
            <div>
              <label className="text-xs font-medium text-[var(--text-secondary)] uppercase">Level</label>
              <p className="text-sm font-medium text-[var(--text)] mt-1">{student?.level || "—"}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium text-[var(--text-secondary)] uppercase">Department</label>
              <p className="text-sm font-medium text-[var(--text)] mt-1">{student?.department?.name || "—"}</p>
            </div>
            <div>
              <label className="text-xs font-medium text-[var(--text-secondary)] uppercase">Faculty</label>
              <p className="text-sm font-medium text-[var(--text)] mt-1">{student?.faculty?.name || "—"}</p>
            </div>
          </div>
          <div>
            <label className="text-xs font-medium text-[var(--text-secondary)] uppercase">Account Status</label>
            <p className="text-sm font-medium text-[var(--text)] mt-1 capitalize">{student?.status || "—"}</p>
          </div>
        </div>
      </Card>
    </div>
